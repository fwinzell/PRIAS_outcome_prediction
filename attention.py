...
def get_feature_dataset(base_dir,
    ...
    features = PRIAS_Feature_Dataset(generic.get_slide_dict(), base_dir, train_mode=False, shuffle=False, return_coords=True)

    return features


def get_patches(attention, coord_dict, k=5, wsi_dir="/home/fi5666wi/PRIAS_data/wsis", display=False):
    ...
    for r, idx in enumerate(wsi_idxs):
        if idx != prev_idx:
            ...
            wsi = imread(wsi_path)
        patch = wsi[pts[r,2]:pts[r,2]+256, pts[r,1]:pts[r,1]+256, :]
...
def get_attention(model, device, base_dir, verbose=True, top_k=None, display=False, save_to_h5=True):
    
    dataset = get_feature_dataset(base_dir, xls_sheet_name=2, use_last_visit=False, use_features=True)
...
if __name__ == "__main__":
    ...
    res_df = get_attention(model.to(device), device, f"{config.base_dir}", verbose=True, top_k=None)
