...
class PRIAS_Generic_Dataset(Dataset):
    ...
    def find_slides_and_labels(self, data_obj):
        ...
        if self.long:
            for i, num in enumerate(self.p_dict['patient_number']):
                ...
                intervals = np.array([0, 3, 6]) * 365  # Multiply by 365 to get days
    ...
    def return_splits(self, set_split=None):
        """
        ...
        if self.val_split > 0:
            ...
            if self.use_features:
                if self.incl_rf:
                    ...
                    tr_split = PRIAS_Feature_Dataset(tr_dict, self.base_dir, train_mode=True, p_gauss=self.p_gauss,
                                                     p_drop=self.p_drop, topk=self.top_k_features)
                    val_split = PRIAS_Feature_Dataset(val_dict, self.base_dir, train_mode=False,
                                                      topk=self.top_k_features)
            else:
                tr_split = PRIAS_Patch_Dataset(tr_dict, self.base_dir, use_last=self.use_last)
                val_split = PRIAS_Patch_Dataset(val_dict, self.base_dir, use_last=self.use_last)
            ...
            if self.use_features:
                if self.incl_rf:
                    ...
                    ret_set = PRIAS_Feature_Dataset(ret_dict, self.base_dir, train_mode=False)
            else:
                ret_set = PRIAS_Patch_Dataset(ret_dict, self.base_dir, use_last=self.use_last)
            return ret_set, None
    ...
    def __getitem__(self, idx):
        ...
        with h5py.File(os.path.join(self.base_dir, f"{slide}.h5"), 'r') as h5df_file:
            ...
            if self.return_coords:
                ...
                ret_dict = {'wsi_name': slide, 'coords': coords}
...
class Simple_Long_Dataset(PRIAS_Generic_Dataset):
    def __init__(self,
                 path_to_dir,
                 xls_path,
                 xls_sheet_name,
        ...
        super().__init__(path_to_dir, xls_path, patient_data_obj, seed=seed, xls_sheet_name=xls_sheet_name,
                         shuffle=True, use_last_visit=False, use_features=True,
                         use_long_labels=True)


    def __getitem__(self, idx):
        pid = self.data_dict['pid'][idx]
        label = self.data_dict['label'][idx]
        slide = self.data_dict['wsi'][idx]
...
class CrossValidation_Dataset(PRIAS_Generic_Dataset):
    ...
    def return_splits(self, K):
        ...
        if self.use_features:
            tr_split = PRIAS_Feature_Dataset(tr_dict, self.base_dir, use_last=self.use_last, train_mode=True,
                                             p_drop=self.p_a[0], p_gauss=self.p_a[1])
            val_split = PRIAS_Feature_Dataset(val_dict, self.base_dir, use_last=self.use_last, train_mode=False)
        else:
            tr_split = PRIAS_Patch_Dataset(tr_dict, self.base_dir, use_last=self.use_last)
            val_split = PRIAS_Patch_Dataset(val_dict, self.base_dir, use_last=self.use_last)
        return tr_split, val_split
...
if __name__ == "__main__":
    ...
    cross_val_dataset = CrossValidation_Dataset(
        path_to_dir="/home/fi5666wi/PRIAS_data/features_densenet",
        patient_data_obj=PDO,
        xls_path="/home/fi5666wi/Documents/PRIAS sheets/PRIAS_from_Edvard_joined_with_AK.xlsx",
        xls_sheet_name=3,
        n_folds=nfolds,
    ...
    dataset = PRIAS_Generic_Dataset(
        path_to_dir="/home/fi5666wi/PRIAS_data/features_densenet_256",
        patient_data_obj=PDO,
    ...
    pids = []
    for i, (pid, inst, label, fac) in enumerate(trloader):
